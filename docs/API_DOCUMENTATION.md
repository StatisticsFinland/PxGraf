...
| GetJsonStat2MetadataAsync | GET: jsonstat/{*sqId*}/metadata?lang={language} | sqId: The id of the saved query provided in the URL. `lang` is optional; when omitted, the table's default language is used. | Single-language JSON-stat 2.0 metadata (`application/vnd.jsonstat2+json`) with an empty `value` array and no `status`. This response bypasses the JSON-stat response cache and does not fetch matrix data for live queries. Deserialized metadata is cached for archived queries and refreshed synchronously when stale. |
