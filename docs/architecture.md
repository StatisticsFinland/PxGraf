...
| `VisualizationController` | `api/sq` | ❌ | `GET visualization/{sqId}` — serve the multilingual visualization response for a saved query (cached with multi-state memory cache); `GET jsonstat/visualization/{sqId}?lang={language}` — serve a single-language JSON-stat 2.0 dataset using the table default when language is omitted and validating explicit languages |
