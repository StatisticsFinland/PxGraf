...
- **Visualization API:** Provides endpoints for fetching visualization data for a saved query given its ID. `GET api/sq/visualization/{id}` returns the existing multilingual `VisualizationResponse`; `GET api/sq/jsonstat/{id}?lang={language}` returns a single-language JSON-stat 2.0 dataset using the shared `JsonStat2` response model. Metadata-only variants are available at `GET api/sq/visualization/{id}/metadata` and `GET api/sq/jsonstat/{id}/metadata?lang={language}`. They omit data points and bypass the visualization response caches; the JSON-stat variant uses an empty `value` array to remain a valid dataset. Both formats share cached deserialized metadata for archived queries. The Creation API also provides `POST api/creation/jsonstat?lang={language}` for JSON-stat preview output. When `lang` is omitted, the endpoints use the table's default language. An explicit unavailable language returns HTTP 400. Both JSON-stat output formats emit the same PxVisualizer `extension.visualizationSettings` schema. More information about the response format can be found in VISUALIZATION_RESPONSE.md
