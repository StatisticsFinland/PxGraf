...
- **Visualization API:** Provides endpoints for fetching visualization data for a saved query given its ID. `GET api/sq/visualization/{id}` returns the existing multilingual `VisualizationResponse`; `GET api/sq/jsonstat/visualization/{id}?lang={language}` returns a single-language JSON-stat 2.0 dataset using the shared `JsonStat2` response model. The Creation API also provides `POST api/creation/jsonstat/visualization?lang={language}` for JSON-stat preview output. When `lang` is omitted, the endpoints use the table's default language. An explicit unavailable language returns HTTP 400. Both JSON-stat endpoints emit the same PxVisualizer `extension.visualizationSettings` schema. More information about the response format can be found in VISUALIZATION_RESPONSE.md
